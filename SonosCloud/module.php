...
class SonosCloud extends IPSModule
{
    ...
    private $apiURL = 'https://api.ws.sonos.com/control/api';
    ...
    public function GetConfigurationForm()
    {
        ...
        if ($this->ReadAttributeString('Token') == '') {
            $data->actions[1]->label = 'Sonos: Pleae register with your Sonos account!';
        } else {
            $result = json_decode(@$this->GetData($this->apiURL . '/v1/households'));
            if ($result === false || $result === null || !isset($result->households)) {
                $data->actions[1]->label = 'Sonos: Error. Please check your internet connection or register again!';
            } else {
                $data->actions[1]->label = 'Sonos: ' . sprintf('Found %d households', count($result->households));
            }
        }
